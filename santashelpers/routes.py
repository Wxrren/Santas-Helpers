...
from flask import logging, request, render_template
from santashelpers import db, app
from santashelpers.models import Activeuser, generate_password_hash
from santashelpers.models import check_password_hash, Christmas_lists
    ...
    if request.method == "POST":
        ...
        if existing_email is True:
            ...
            return render_template(
                'register.html', error="Username already registered.")
        # Checking for existing users username
        existing_username = Activeuser.query.filter_by(
        ...
        flash(
            'You have successfully registered! Feel free to sign in.',
            'success')
...
def sign_in():
    if request.method == "POST":
        ...
        # Search for the user
        existing_site_user = Activeuser.query.filter_by(
            username=username).first()

        # Check the user exists and the details are correct
        if existing_site_user and check_password_hash(
                existing_site_user.password,
                password):
            # success
            ...
            flash("Oh no! We don't recognise the username or password",'error')
...
def my_list():
    ...
    if 'user_id' not in session:
        ...
        return redirect(url_for('sign_in'))
    # Search for the current user logged in the session.
    # query their lists.
    current_user_id = session['user_id']

    current_user_lists = Christmas_lists.query.filter_by(
        owner_id=current_user_id).all()

    return render_template(
        "my_list.html", current_user_lists=current_user_lists)
...
def add_list():
    if request.method == "POST":
        ...
        letter_to_santa = True if request.form.get(
            "letter_to_santa") == 'on' else False
        milk_and_cookies = True if request.form.get(
            "milk_and_cookies") == 'on' else False
...
def edit_list(list_id):
    ...
    if request.method == "POST":
        ...
        edit_user_list.user_christmas_list = request.form.get(
            "user_christmas_list")
        edit_user_list.letter_to_santa = True if request.form.get(
            "letter_to_santa") == 'on' else False
        edit_user_list.milk_and_cookies = True if request.form.get(
            "milk_and_cookies") == 'on' else False
        edit_user_list.favourite_reindeer = request.form.get(
            "favourite_reindeer")
        db.session.commit()
...
def search():
    ...
    users = Activeuser.query.filter(Activeuser.username.ilike(
        f'%{search_user}%')).all()
